...
@section('header_title', 'Pillars of the Confluence')

@section('content')
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <h2 style="color: var(--admin-sidebar); font-size: 1.5rem;">Pillars of the Confluence</h2>
        <p style="color: #64748b; font-size: 0.9rem;">Manage the content for the Five Pillars section.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.pillars.update') }}" enctype="multipart/form-data">
        @csrf
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 30px;">
            
            @foreach(['pillars' => 'Five Pillars Settings'] as $groupKey => $groupName)
                            ...
                            <div style="margin-bottom: 20px;">
                                ...
                                @elseif($setting->type == 'image' || str_contains($setting->key, '_icon'))
                                    @if($setting->value && str_starts_with($setting->value, 'fa-'))
                                        ...
                                        <small style="color: #64748b; display: block; margin-top: 5px;">Enter a FontAwesome class (e.g. fa-solid fa-laptop-medical) or upload an image below to replace it.</small>
